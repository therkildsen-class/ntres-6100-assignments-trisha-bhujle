---
title: "Assignment 8: Iteration and conditional execution"
format: gfm
editor: visual
---
...
Answer: *Write your response here.* The code chunks have very similar formatting. They all use the read_csv function to read in the data frame, and they also share the exact same phrase "na = c("99", "999", "99.00", "999.0")". Looking at the data frames themselves, they and only differ in their values for the first column, YY (which is 87, 88, 89, and 90).

The only parts of the code chunks that are different are the ends of the urls to import the csv file. All four data frames are derived from buoy 44013, and the only thing that differs is the "\_year" after 44013.
...
-   The first column (year) followed a two-digit format from 1987 to 1998, but has (understandably) switched to a four-digit format since 1999.
...
-   don’t worry about parsing failures in columns other than the ones that you will use for this exercise
...
Vectorization seems to work best when you only have to perform a single operation once. For example, when we were estimating the mass of a Spinosaurus that is 16m long based on its reassembled skeleton, we already had all of the numbers we needed to answer the question in a single operation. Similarly, when we needed to calculate the mass of many dinosaurs whose a_values, lengths, and b_values were defined, vectorization enabled the calculation to be completed within seconds. For loops are preferable in situations where you have to perform multiple more complex operations, such as importing several data frames and editing them in a single loop or making several edits to several data frames and then making a graph from those edits. For loops are also especially useful because if you have to change one term in the loop, the rest of the loop's outputs will update themselves after you run it!
