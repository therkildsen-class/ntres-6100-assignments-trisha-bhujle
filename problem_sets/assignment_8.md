...
z \<- NULL for (i in 1:length(x)){ z \<- c(z, x\[i\] + y\[i\])
...
``` r
mass_vect <- a_values * dinosaur_lengths ^ b_values
mass_vect
```
...
for (i in seq_along(dinosaur_lengths)) {
  mass_forloop <- a_values[i] * dinosaur_lengths[i] ^ b_values[i]
}
mass_forloop
```
...
``` r
system.time(mass_vect <- a_values * dinosaur_lengths ^ b_values)
```
...
``` r
system.time(for (i in seq_along(dinosaur_lengths)) {
  mass_forloop[i] <- a_values[i] * dinosaur_lengths[i] ^ b_values[i]
})
```
...
Answer: *Write your response here.* The code chunks have very similar
formatting. They all use the read_csv function to read in the data
frame, and they also share the exact same phrase “na = c(”99”, “999”,
“99.00”, “999.0”)“. Looking at the data frames themselves, they and only
differ in their values for the first column, YY (which is 87, 88, 89,
and 90).

The only parts of the code chunks that are different are the ends of the
urls to import the csv file. All four data frames are derived from buoy
44013, and the only thing that differs is the “\_year” after 44013.
...
for (year in start:end){
  path <- str_c("https://raw.githubusercontent.com/nt246/NTRES-6100-data-science/master/datasets/buoydata/44013_", year, ".csv", sep = "")
  df <- read_csv(path, na = c("99", "999", "99.00", "999.0"), show_col_types = FALSE) |>
  select(YY, MM, WVHT, WTMP) |>
  rename(year = YY, month = MM, wave_heights = WVHT, temperatures = WTMP) |>
  group_by(month) |>
  mutate(avg_height = mean(wave_heights), avg_temp = mean(temperatures))
  
  df_combined <- bind_rows(df_combined, df)
  
  wave_height_plot <- ggplot(data = df_combined, mapping = aes(x = year, y = avg_height)) +
  geom_point() +
  labs(title = "Monthly average wave height from 1987 to 1992")
  
  ggsave(filename = "wave_height_plot.png", plot = wave_height_plot)

  temp_plot <- ggplot(data = df_combined, mapping = aes(x = year, y = avg_temp)) +
  geom_point() +
  labs(title = "Monthly average temperature from 1987 to 1992")
  
  ggsave(filename = "temp_plot.png", plot = temp_plot)
}
```
...
Vectorization seems to work best when you only have to perform a single
operation once. For example, when we were estimating the mass of a
Spinosaurus that is 16m long based on its reassembled skeleton, we
already had all of the numbers we needed to answer the question in a
single operation. Similarly, when we needed to calculate the mass of
many dinosaurs whose a_values, lengths, and b_values were defined,
vectorization enabled the calculation to be completed within seconds.
For loops are preferable in situations where you have to perform
multiple more complex operations, such as importing several data frames
and editing them in a single loop or making several edits to several
data frames and then making a graph from those edits. For loops are also
especially useful because if you have to change one term in the loop,
the rest of the loop’s outputs will update themselves after you run it!
