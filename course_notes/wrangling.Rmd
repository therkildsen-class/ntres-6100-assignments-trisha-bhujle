...
# Don't pipe within the ggplot function—use a plus sign instead! 
```
...
```{r} 
# First, find the top 5 countries and save them as an object. 
coronavirus |>
  filter(type == "confirmed") |>
  group_by(country) |>
  summarize(total = sum(cases)) |>
...
# Then, use that information to make a ggplot.
coronavirus |>
  ...
  facet_wrap(~country) # Could add nrow = or ncol = to customize.
# You could also delete the legend because it is redundant.
```
